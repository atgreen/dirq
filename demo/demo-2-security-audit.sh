...
echo ""
echo '$ dirq query "SELECT os_info.hostname, packages.name, packages.version FROM * WHERE packages.name = '"'"'openssl'"'"'"'
$DIRQ query "SELECT os_info.hostname, packages.name, packages.version FROM * WHERE packages.name = 'openssl'"
echo ""

read -p "Press Enter to find hosts missing curl (bare installs)..."
echo ""
echo "Querying for hosts that DO have curl..."
echo '$ dirq query "SELECT os_info.hostname FROM * WHERE packages.name = '"'"'curl'"'"'"'
HAVE_CURL=$($DIRQ query "SELECT os_info.hostname FROM * WHERE packages.name = 'curl'" --json | python3 -c "import sys,json; r=json.load(sys.stdin); print(', '.join([x['hostname'] for x in r['results']]))" 2>/dev/null)
...
echo ""
echo '$ dirq query "SELECT os_info.hostname, packages.name, packages.version FROM tag:env=prod WHERE packages.name IN ('"'"'nginx'"'"', '"'"'openssl'"'"', '"'"'curl'"'"')"'
$DIRQ query "SELECT os_info.hostname, packages.name, packages.version FROM tag:env=prod WHERE packages.name IN ('nginx', 'openssl', 'curl')"
echo ""
