...
echo '$ dirq query "SELECT os_info.hostname WHERE packages.name = '"'"'curl'"'"'" --json'
$DIRQ query "SELECT os_info.hostname WHERE packages.name = 'curl'" --json 2>/dev/null | python3 -c "
import sys, json
...
echo '$ dirq run --query "SELECT os_info.hostname" --command "dnf install -y curl" --forks 3'
$DIRQ run --query "SELECT os_info.hostname" --command "dnf install -y curl" --forks 3
echo ""

read -p "Press Enter to verify curl is now everywhere..."
echo ""
echo 'Step 3: Verify'
echo '$ dirq query "SELECT os_info.hostname, packages.name, packages.version WHERE packages.name = '"'"'curl'"'"'"'
$DIRQ query "SELECT os_info.hostname, packages.name, packages.version WHERE packages.name = 'curl'"
echo ""
