...
// WithTopologyLock runs fn while holding the topology mutex.
// SQLite is single-server, so a Go mutex replaces pg_advisory_xact_lock.
func (d *DB) WithTopologyLock(_ context.Context, fn func() error) error {
	d.topologyMu.Lock()
...
func (d *DB) FindRelaysWithChildren(ctx context.Context) ([]db.Agent, error) {
	rows, err := d.db.QueryContext(ctx, `
		...
		FROM agents a
		WHERE a.role = 'relay' AND a.online = 1 AND a.listen_addr != ''
		  AND (SELECT COUNT(*) FROM agents c WHERE c.parent_id = a.id AND c.online = 1) > 0
		ORDER BY (SELECT COUNT(*) FROM agents c WHERE c.parent_id = a.id AND c.online = 1) DESC
		LIMIT 5`)
	if err != nil {
		return nil, err
	}
...
func (d *DB) FindImbalancedNodes(ctx context.Context, maxChildren int) (heavy db.NodeLoad, light db.NodeLoad, found bool, err error) {
	row := d.db.QueryRowContext(ctx, `
		SELECT a.id, a.hostname, a.os, a.os_version, a.arch, a.agent_version,
		       ...
		       a.parent_id, a.server_pod, a.online, a.exec_enabled,
		       a.registered_at, a.last_seen_at,
		       (SELECT COUNT(*) FROM agents c WHERE c.parent_id = a.id AND c.online = 1) AS child_count
		FROM agents a
		WHERE a.online = 1 AND a.listen_addr != ''
		  AND (SELECT COUNT(*) FROM agents c WHERE c.parent_id = a.id AND c.online = 1) > 1
	...
	scanErr := row.Scan(
		...
		&capsJSON, &tagsJSON,
		&parentID, &serverPod, &online, &execEnabled,
	...
	heavyAgent.Online = online != 0
	heavyAgent.ExecEnabled = execEnabled != 0
	if parentID.Valid {
		heavyAgent.ParentID = &parentID.String
	}
	if serverPod.Valid {
		heavyAgent.ServerPod = &serverPod.String
	}
	json.Unmarshal([]byte(capsJSON), &heavyAgent.Capabilities)
	json.Unmarshal([]byte(tagsJSON), &heavyAgent.Tags)
	if heavyAgent.Tags == nil {
		heavyAgent.Tags = make(map[string]string)
	}
	if heavyAgent.Capabilities == nil {
		heavyAgent.Capabilities = []string{}
	}
	heavyAgent.RegisteredAt, _ = time.Parse(time.RFC3339, registeredAt)
	heavyAgent.LastSeenAt, _ = time.Parse(time.RFC3339, lastSeenAt)
...
func (d *DB) FindChildOfParent(ctx context.Context, parentID string) (db.Agent, error) {
	row := d.db.QueryRowContext(ctx, `
		SELECT a.id, a.hostname, a.os, a.os_version, a.arch, a.agent_version,
		       ...
		       a.registered_at, a.last_seen_at
		FROM agents a
		WHERE a.parent_id = ? AND a.online = 1 AND a.listen_addr != ''
		ORDER BY (SELECT COUNT(*) FROM agents c WHERE c.parent_id = a.id AND c.online = 1) DESC
		LIMIT 1`,
		parentID,
	)
...
// CountOnlineZoneLeaders returns the number of online zone leaders.
func (d *DB) CountOnlineZoneLeaders(ctx context.Context) (int, error) {
	var count int
	err := d.db.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM agents WHERE role = 'zone_leader' AND online = 1`,
	).Scan(&count)
	return count, err
}
...
func (d *DB) CountAgentsByRole(ctx context.Context, role string) (int, error) {
	var count int
	err := d.db.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM agents WHERE role = ? AND online = 1`, role,
	).Scan(&count)
	return count, err
}

// CountChildren returns the number of online children of the given agent.
func (d *DB) CountChildren(ctx context.Context, parentID string) (int, error) {
	var count int
	err := d.db.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM agents WHERE parent_id = ? AND online = 1`, parentID,
	).Scan(&count)
	return count, err
}
	...
	rows, err := d.db.QueryContext(ctx, `
		WITH RECURSIVE tree AS (
			SELECT id, hostname, os, os_version, arch, agent_version,
			       ...
			       id AS root_zl
			FROM agents
			WHERE role = 'zone_leader' AND online = 1 AND listen_addr != ''
			UNION ALL
			SELECT a.id, a.hostname, a.os, a.os_version, a.arch, a.agent_version,
			       a.listen_addr, a.role, a.capabilities, a.tags,
			       a.parent_id, a.server_pod, a.online, a.exec_enabled,
		...
		FROM tree t
		WHERE t.id != ?
		  AND (SELECT COUNT(*) FROM agents c WHERE c.parent_id = t.id AND c.online = 1) < ?
		ORDER BY
		  CASE WHEN t.root_zl = (
		    SELECT root_zl FROM tree WHERE id = ? LIMIT 1
		  ) THEN 1 ELSE 0 END ASC,
		  t.depth ASC,
		  (SELECT COUNT(*) FROM agents c WHERE c.parent_id = t.id AND c.online = 1) ASC
		LIMIT ?`,
		primaryParentID, maxChildren, primaryParentID, count,
	)
	if err != nil {
		return nil, err
	}
...
func (d *DB) FindParentWithRoom(ctx context.Context, role string, maxChildren int) (db.Agent, error) {
	row := d.db.QueryRowContext(ctx, `
		SELECT a.id, a.hostname, a.os, a.os_version, a.arch, a.agent_version,
		       ...
		       a.registered_at, a.last_seen_at
		FROM agents a
		WHERE a.role = ? AND a.online = 1 AND a.listen_addr != ''
		  AND (SELECT COUNT(*) FROM agents c WHERE c.parent_id = a.id AND c.online = 1) < ?
		ORDER BY (SELECT COUNT(*) FROM agents c WHERE c.parent_id = a.id AND c.online = 1) ASC
		LIMIT 1`,
		role, maxChildren,
	)
...
// FindShallowestParentWithRoom finds the shallowest node with room for a child.
func (d *DB) FindShallowestParentWithRoom(ctx context.Context, maxChildren int) (db.Agent, error) {
	row := d.db.QueryRowContext(ctx, `
		...
		SELECT t.id, t.hostname, t.os, t.os_version, t.arch, t.agent_version,
		       ...
		       t.registered_at, t.last_seen_at
		FROM tree t
		WHERE (SELECT COUNT(*) FROM agents c WHERE c.parent_id = t.id AND c.online = 1) < ?
