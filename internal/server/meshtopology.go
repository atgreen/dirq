...
// FindImbalanced returns one heavy and one light online node whose loads
// are at least 2× apart.  Used by the rebalancer's redistributeOne path.
func (t *MeshTopology) FindImbalanced() (heavy, light db.NodeLoad, found bool) {
	t.mu.RLock()
	defer t.mu.RUnlock()

	// heaviest first
	var heavyID, lightID string
	heavyCount, lightCount := 0, t.cfg.MaxChildrenPerNode+1
	for id, n := range t.nodes {
		...
		c := 0
		for childID := range n.children {
			if cn, ok := t.nodes[childID]; ok && cn.online {
				c++
			}
		}
		if c > heavyCount {
		...
		if c < lightCount && c < t.cfg.MaxChildrenPerNode {
			lightCount = c
			lightID = id
		}
	}
	if heavyID == "" || lightID == "" || heavyID == lightID {
		return heavy, light, false
	}
	...
	heavy = db.NodeLoad{
		Agent: db.Agent{
			ID: hn.id, Hostname: hn.hostname, ListenAddr: hn.listenAddr, Role: hn.role,
		},
		ChildCount: heavyCount,
	}
	light = db.NodeLoad{
		Agent: db.Agent{
			ID: ln.id, Hostname: ln.hostname, ListenAddr: ln.listenAddr, Role: ln.role,
		},
		ChildCount: lightCount,
	}
	return heavy, light, true
}

// PickChildOf returns one online child of parentID, preferring the one
// with the largest subtree (used when moving a subtree to balance load).
func (t *MeshTopology) PickChildOf(parentID string) (db.Agent, bool) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	p, ok := t.nodes[parentID]
	if !ok {
