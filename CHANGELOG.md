...
- **The exec audit log is admin-only.** `/api/v1/exec_log` accepted a readonly token, and every row in it carries the command line and the source and destination paths of a privileged operation — command lines that routinely carry inline credentials. The documented scope table never claimed otherwise; the route did. Reading it now requires admin, like the operations it records.
- **The server no longer writes credentials to its log.** The line announcing the database backend printed the DSN in full on every boot, password included; it is redacted now. And when the first-boot bootstrap token could not be written to its 0600 file, the server fell back to logging the token — an admin credential — in plaintext. It refuses to start instead, which is no worse off operationally, since without that file there is no usable token either way.
- **A cycle in the agents table can no longer wedge the server.** The topology refuses to record a parent cycle in memory, but the database mirror is written node by node with no transaction, so a parent swap observed mid-snapshot can persist one that never existed. The recursive walks over `parent_id` then ran forever, and because the cycle is on disk, a restart hit the same wall. Both walks are depth-capped, and the caller that had no deadline at all now has one. (The snapshot itself is still not atomic — that is tracked separately.)
- **The registration secret is compared in constant time.** It is the only gate on joining the mesh; the same codebase already took that care when pinning the server signing key.
- **The banking example policy fails closed on host classification.** `examples/policy/aap-banking.rego` derived "this is a production or PCI host" from a tag having to be present, so a host with no `env` tag fell through to the *permissive* branch — with `allow` still true, so nothing looked wrong. A host is now high-assurance unless it explicitly says otherwise. The tags a policy sees are the agent's own `agent.conf`, never the server-side tags set with `dirq hosts tag`, and the example now says so: classify hosts at provisioning time or the policy cannot see the classification at all.
- **The AWS demo fleet no longer ships with published credentials or an open front door.** `demo/aws-test-fleet.sh` defaulted the Windows Administrator password and the agent registration secret to literals committed in a public repo, and opened SSH, RDP, Grafana and the DirQ API to `0.0.0.0/0`. Both secrets are now generated per fleet and kept in `.dirq-aws-state/`, management ports are restricted to `DIRQ_ALLOWED_CIDR` (defaulting to the operator's own address), and the mesh ports are reachable only from the fleet's own security group.
- **The server refuses a data directory it does not privately own.** When `/var/lib/dirq` is not writable, DirQ falls back to a directory under `/tmp` — and `MkdirAll` succeeds on a directory that already exists, leaving its owner and mode alone. On a shared host a local user could create that path first and own the subtree the server then wrote its Ed25519 signing key, TLS material and bootstrap token into; owning the parent is enough to swap any of them whatever mode the files carry. The fallback is now used only if it is a real directory, owned by this user, not group- or world-writable. If neither location can be trusted the server says so and fails on the write rather than storing a signing key somewhere someone else controls.
- **The agent's relay listener fails closed.** If TLS credentials could not be loaded, the agent logged the error and served the relay port in plaintext — losing encryption and the client-certificate binding that ties a downstream peer to its agent ID, leaving a bearer token as the only gate. It now refuses to serve, the way the dial side already refused to connect.
- **The Ansible collection can verify a private CA.** Its shared HTTP client had two modes — system trust store, or no verification at all — and `dirq cert generate` produces a CA that is in neither, so the only working configuration against a default deployment was to turn verification off for every request. Every one of those carries your DirQ API token. The connection, inventory and cache plugins now take `tls_ca` / `DIRQ_TLS_CA`, the same variable the server, agents and CLI already read, and the README recommends it instead of `DIRQ_TLS_INSECURE`. The standalone connection plugin already had this; the collection was missed.
- **A snapshot of the mesh now describes one instant.** The topology was persisted node by node, re-taking the lock for each, so a parent swap landing between two reads could write a cycle to the agents table that never existed in memory — where the live topology refuses to record one. Combined with the depth caps above, the shape that wedged the server can no longer be created in the first place.
- **The server runs as a `dirq` system account.** The packaged unit ran as root for no reason: unprivileged ports, and nothing to write outside its state directory. It now runs unprivileged with `NoNewPrivileges`, `ProtectSystem=strict`, `PrivateTmp` and a systemd-managed `StateDirectory`, and the container image drops to UID 1001. Upgrades hand `/var/lib/dirq` to the new account. The **agent** deliberately stays root — it runs operator-authorized commands on the host and escalates with sudo for `become`, and the controls that bound it are `exec_enabled`, token scopes and the agent-side policy, not the unit file.
- **An abandoned query stops costing the fleet.** Agent-side filtering took no context, so when the server's query timeout expired every agent carried on filtering for a query nobody was waiting for; the server's own fleet-wide pre-filter ignored client cancellation the same way. Both now stop when the caller has given up.
- **A command is no longer delivered to every agent in the fleet.** A single-host `exec`, `put_file` or `fetch_file` was fanned out to every connected zone leader whenever the target was not directly connected, and each relay then copied it to *all* of its children rather than the one that leads to the target. Every agent in the path's zone — and in fan-out, the whole fleet — received the command line, the script, the stdin and the file body. That matters most for the Ansible integration, where each task is a per-host exec and module arguments carry whatever the playbook decrypted, so one compromised host could read the credentials being pushed to every other host. The server now computes the route to the target and sends it to that zone leader alone, the request carries that route, and each relay forwards to the next hop only. Where no route can be traced — the topology does not know the agent, or its zone leader has no live stream — the old fan-out still happens so an exec does not fail because the tree moved, and `dirq_exec_fanout_total` counts how often that is. Broadcast `exec_multi` and `deploy` are unchanged for now: they address a set of hosts, so there is no single path to follow.
- **A `LIKE` pattern can no longer pin every CPU in the fleet.** The matcher recursed over every suffix position at each `%`, so each added wildcard multiplied the work by roughly the length of the value being matched. Against a 50-byte package name, eight wildcards took half a second, twelve took a hundred, and sixteen would have run for hours. The pattern comes from a **readonly** API token, and it is evaluated once per element of every array module the query touches — every installed package, every service — on every agent the query reaches, so one 40-byte query was enough to saturate a core on every managed host. Nothing bounded it: not the request body cap, not the parser's nesting limit, not the broadcast rate limit, and not the query timeout, which expires on the server while the agents keep spinning. Matching is now linear in the length of the value and the pattern, and patterns are capped at 1024 bytes — roughly ten times any real package or hostname glob. Matching semantics are unchanged, checked exhaustively against the old implementation.
- **Agents can no longer speak for one another.** A zone leader's stream carries its whole subtree multiplexed onto one connection, so the mTLS certificate authenticates the zone leader and nothing else; every message names its own origin in a field, and the server believed the field. Any agent in the mesh could therefore answer another host's `exec`, `put_file` or `fetch_file` — responses were matched on request id alone, and the first answer won — claim a victim's slot in a broadcast so the victim's real reply was discarded as a duplicate, overwrite any host's facts and hide vulnerable packages from a CVE scan, or mark any subtree offline, which no amount of waiting undoes because nothing brings an agent back online until it reconnects while every broadcast skips it meanwhile. The server now checks each claim against the stream it arrived on: a response must name the agent its request was dispatched to, a claimed origin must be the sending stream or something beneath it in the mesh, and a relay may only report attachments to itself and the loss of a child it actually had. A compromised leaf — the common case — is now confined to speaking for itself. This is on by default — `agent_origin_checks` (`DIRQ_AGENT_ORIGIN_CHECKS`) enforces unless you set it to `observe`, which counts violations in `dirq_agent_origin_violations_total` without dropping anything. Legitimate traffic should not trip it: a relay announces a child before it forwards anything from that child, and streams preserve order, so the server knows where an agent sits before its messages arrive. What enforcement does drop is a message still in flight down a path the agent has already left — stale by definition, and counted in the metric either way. A compromised relay or zone leader can still speak for its own subtree; closing that needs per-message signing, which is the next step.
